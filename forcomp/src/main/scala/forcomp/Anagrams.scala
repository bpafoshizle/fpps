...
object Anagrams {
  ...
  def combinations(occs: Occurrences): List[Occurrences] = {
     val explodedOccs = for {
        occ <- occs
        i <- 1 to occ._2
     } yield (occ._1, i)

     def subsets(sets: Occurrences): List[List[(Char, Int)]] = {
        sets match {
           case Nil => List(List())
           case List((c: Char, i: Int)) => {
              List(List()) ++ List(List((c, i)))
           }
           case (c: Char, i: Int) :: xs => {
              List(List((c, i))) ++ subsets(xs) ++ {
                 for {
                    set <- subsets(xs)
                    if !set.isEmpty && c < set.head._1
                 } yield ((c, i) :: set)
              }
           }
        }
     }
     subsets(explodedOccs)
  }
